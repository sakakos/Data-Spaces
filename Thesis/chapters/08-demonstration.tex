...
\textlatin{Kong API Gateway} εκτίθεται στη θύρα \texttt{8000} του τοπικού
μηχανήματος και αποτελεί το μοναδικό σημείο εισόδου για όλα τα αιτήματα
δεδομένων. Ο \textlatin{Keyrock} εκτίθεται στη θύρα \texttt{3007} για τη
διαδικασία ανταλλαγής \textlatin{token}.

Τα εργαλεία που χρησιμοποιούνται για την επίδειξη είναι:
\begin{itemize}
    \item \textbf{\textlatin{PowerShell}}: Για την εκτέλεση των εντολών
    επίδειξης.
    \item \textbf{\textlatin{curl}}: Για την αποστολή \textlatin{HTTP}
    αιτημάτων.
    \item \textbf{\textlatin{Docker}}: Για την εκτέλεση των \textlatin{scripts}
    δημιουργίας \textlatin{JWT}.
    \item \textbf{\textlatin{Python}}: Για την παρουσίαση των αποτελεσμάτων
    σε \textlatin{JSON} μορφή (\texttt{python -m json.tool}).
\end{itemize}
...
\begin{lstlisting}[language=bash, caption={Αίτημα χωρίς \textlatin{token} ταυτοποίησης.}]
curl http://localhost:8000/ngsi-ld/v1/entities \
     ?type=https://plegma.example.org/vocab%23HouseholdElectricMeasurement \
     &limit=1
\end{lstlisting}

\noindent \textbf{Αναμενόμενο αποτέλεσμα:}
\begin{lstlisting}[language=bash, caption={Απόκριση \textlatin{Kong} για μη ταυτοποιημένο αίτημα.}]
HTTP/1.1 401 Unauthorized
{"message": "Unauthorized"}
\end{lstlisting}

Το \textlatin{Kong} επιστρέφει \textlatin{HTTP 401 Unauthorized} καθώς
δεν υπάρχει κεφαλίδα \texttt{Authorization} στο αίτημα. Το αίτημα
...
\begin{lstlisting}[language=bash, caption={Δημιουργία \textlatin{iSHARE JWT} μέσω \textlatin{Docker}.}]
# Το script gen_token.py εκτελείται μέσα σε container Python
# ώστε να έχει πρόσβαση στα certificates χωρίς τοπική εγκατάσταση
...
\begin{itemize}
    \item \texttt{-{}-rm}: Αφαιρεί το \textlatin{container} μετά την εκτέλεση.
    \item \texttt{-v "./iShare:/iShare"}: Προσαρτά τον φάκελο με τα
    πιστοποιητικά στο \textlatin{container}.
    \item \texttt{pip install PyJWT cryptography -q 2>/dev/null}: Εγκαθιστά
    τις απαραίτητες βιβλιοθήκες αθόρυβα.
    \item \texttt{python3 /iShare/gen\_token.py}: Εκτελεί το \textlatin{script}
    δημιουργίας \textlatin{JWT}.
\end{itemize}
...
\begin{lstlisting}[language=bash, caption={Αίτημα ανάγνωσης μετρήσεων ηλεκτρισμού με \textlatin{token} παρόχου.}]
curl http://localhost:8000/ngsi-ld/v1/entities \
     ?type=https://plegma.example.org/vocab%23HouseholdElectricMeasurement \
     &limit=1 \
...
\begin{lstlisting}[language=bash, caption={Απόκριση με δεδομένα μέτρησης ηλεκτρισμού.}]
[
  {
...
\begin{lstlisting}[language=bash, caption={Αίτημα ανάγνωσης περιβαλλοντικών μετρήσεων.}]
curl http://localhost:8000/ngsi-ld/v1/entities \
...
\noindent \textbf{Αποτέλεσμα (\textlatin{HTTP 200 OK}):}
\begin{lstlisting}[language=bash, caption={Απόκριση με δεδομένα περιβαλλοντικών μετρήσεων.}]
[
  {
...
\begin{lstlisting}[language=bash, caption={Αίτημα ανάγνωσης δεδομένων κτιρίου.}]
curl http://localhost:8000/ngsi-ld/v1/entities \
...
\noindent \textbf{Αποτέλεσμα (\textlatin{HTTP 200 OK}):}
\begin{lstlisting}[language=bash, caption={Απόκριση με μεταδεδομένα κτιρίου.}]
[
  {
...
στον καταναλωτή να διαβάζει δεδομένα τύπου \texttt{HouseholdElectricMeasurement},
...
\begin{lstlisting}[language=bash, caption={Δημιουργία \textlatin{iSHARE JWT} καταναλωτή.}]
...
\begin{lstlisting}[language=bash, caption={Ανταλλαγή \textlatin{iSHARE JWT} με \textlatin{access token}.}]
...
\begin{lstlisting}[language=bash, caption={Αίτημα ανάγνωσης δεδομένων από τον καταναλωτή.}]
curl http://localhost:8000/ngsi-ld/v1/entities \
     ?type=https://plegma.example.org/vocab%23HouseholdElectricMeasurement \
     &limit=1 \
...
\noindent \textbf{Αποτέλεσμα (\textlatin{HTTP 200 OK}):}
\begin{lstlisting}[language=bash, caption={Επιτυχής ανάγνωση δεδομένων από τον καταναλωτή.}]
[
  {
    "id": "urn:ngsi-ld:HouseholdElectricMeasurement:House01:2022-07-15T00:00:00Z",
    "type": "https://plegma.example.org/vocab#HouseholdElectricMeasurement",
...
\begin{tabular}{|p{0.5cm}|p{5cm}|p{2.5cm}|p{5cm}|}
\hline
\textbf{\#} & \textbf{Σενάριο} & \textbf{Αποτέλεσμα} & \textbf{Αιτιολογία} \\
\hline
1 & Χωρίς \textlatin{token} & \textlatin{401 Unauthorized} & Απουσία κεφαλίδας \texttt{Authorization} \\
\hline
...
\begin{itemize}
    \item \textbf{Μηδενική πρόσβαση χωρίς ταυτοποίηση}: Κανένα αίτημα
    χωρίς έγκυρο \textlatin{token} δεν φτάνει στα δεδομένα.
    \item \textbf{Πλήρης πρόσβαση παρόχου}: Ο πάροχος έχει πρόσβαση σε
    όλα τα δεδομένα που κατέχει, αποδεικνύοντας ταυτότητα μέσω
    ψηφιακής υπογραφής.
    \item \textbf{Ελεγχόμενη πρόσβαση καταναλωτή}: Ο καταναλωτής αποκτά
...
Στο παρόν κεφάλαιο παρουσιάστηκε πειραματικά ο κύκλος εκχώρησης πρόσβασης
του \textlatin{Plegma Data Space} μέσω πέντε σεναρίων επίδειξης. Επαληθεύτηκε
η ορθή λειτουργία του ελέγχου πρόσβασης τόσο για τον πάροχο όσο και για τον
καταναλωτή, καθώς και η αποτελεσματική απόρριψη μη εξουσιοδοτημένων αιτημάτων.

Στο τελευταίο κεφάλαιο παρουσιάζονται τα συμπεράσματα της εργασίας και οι
